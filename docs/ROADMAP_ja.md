...
- [x] ロードマップ（本ドキュメント）
- [ ] `uv init` によるプロジェクト骨格の作成
- [ ] `pyproject.toml` の依存関係宣言
- [ ] サンプル `tasks.json`（フィクスチャ）
- [ ] `CLAUDE.md` プロジェクトレベルの開発規約
- [ ] CI: GitHub Actions（lint + 型チェック）
...
- [ ] Textual App 基盤フレームワーク (`app.py`)
- [ ] ヘッダーウィジェット: タブ切り替え（Today / Week / Month / Inbox）
- [ ] TaskList ウィジェット: 優先度別グループ表示
- [ ] TaskRow ウィジェット: チェックボックス + タイトル + タグ + 優先度スター
- [ ] フッターウィジェット: タスク統計情報
- [ ] データ層: Task スキーマの pydantic モデル
- [ ] データ層: `tasks.json` のパース
- [ ] 基本キーボードナビゲーション: `j/k` 上下移動, `q` 終了
- [ ] サブタスクのレンダリング: インデントされた子要素（最大3階層）
- [ ] 空状態のプレースホルダーページ
...
- [ ] watchdog 統合: `tasks.json` の変更を監視
- [ ] デバウンス機構（100ms）
- [ ] ホットリロード: JSON の変更 → ちらつきのない TUI 再レンダリング
- [ ] マウスクリックによるチェックボックス → ステータスの切り替え → JSON への書き戻し
- [ ] アトミック書き込み（一時ファイル + `os.rename`）
- [ ] スキーマバリデーション: 不正な JSON → エラーバナー表示
- [ ] エラー状態: ファイルの欠落や不正形式に対するグレースフルデグラデーション
- [ ] 詳細パネル: `Enter` でタスク詳細を展開
- [ ] エンドツーエンドシナリオ: エージェントが別のターミナルで JSON を書き込む → TUI が自動更新
