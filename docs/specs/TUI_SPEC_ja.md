...
フォーカス中のタスクで確認操作を行うと（デフォルトでは `Enter`）、またはそのタイトルをクリックすると、詳細パネルが開きます。
...
すべての preset は semantic-first なインタラクションモデルを共有します。

- `Enter` は `confirm` にマップされる
- `Escape` は `back` にマップされる
- `n` は `create` にマップされる
- `e` は `edit` にマップされる

現在のメイン task board では、`confirm` はフォーカス中の task detail を開き、`back` は confirm bar、search、detail の順で最上位レイヤーを閉じます。
...
- task form を開く
- `title` は必須
- 複数の writable source がある場合、フォームは `source/project -> scope -> task fields` を明示的に表示する
- そうでない場合、フォームは現在の writable source に直接 submit する
- 適用可能な場合、scope は現在の filter context をデフォルトにする
- subtask は親 task の source に固定され、source selector は表示しない
