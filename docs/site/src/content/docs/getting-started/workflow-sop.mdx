...
| **tmux** | Split panes, persist sessions | `apt install tmux` / `brew install tmux` |
...
<Tabs>
  <TabItem label="Windows">
    ...
    Set your Windows Terminal default profile to **Ubuntu (WSL)**.

    ### Step 3: Install tools inside WSL
...
<Tabs>
  ...
  <TabItem label="PowerShell (WSL)">
    ```powershell
    # mu.ps1 — Windows shortcut that calls WSL
    ...
    $wslPath = wsl wslpath -u (Resolve-Path $ProjectDir).Path

    # Launch in WSL
    wsl bash -lc "~/.local/bin/mu '$wslPath' '$Agent'"
    ```
  </TabItem>
</Tabs>
...
| WSL path issues | Use `wslpath` to convert paths; the PowerShell script handles this |
