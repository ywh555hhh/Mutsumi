...
**触发方式**：按 <kbd>n</kbd> 键或点击底部 `[+New]` 按钮。
...
<Aside type="tip">
在 Today 标签下按 <kbd>n</kbd> 创建的新任务，scope 会自动设为 `day`。
</Aside>

## 编辑任务

**触发方式**：按 <kbd>e</kbd> 键打开编辑表单，或按 <kbd>i</kbd> 键进行行内编辑。
...
**触发方式**：按 <kbd>Space</kbd> 键或鼠标点击复选框。
...
按 <kbd>A</kbd> (Shift+a) 为选中任务添加子任务。也可以在详情面板中点击 `[+Sub]` 按钮。
...
| 按键 | 操作 |
|---|---|
| <kbd>J</kbd> (Shift+j) | 在列表中下移任务 |
| <kbd>K</kbd> (Shift+k) | 在列表中上移任务 |
...
| <kbd>h</kbd> | 折叠优先级分组 |
| <kbd>l</kbd> | 展开优先级分组 |
| <kbd>z</kbd> | 切换子任务折叠/展开 |

## 排序

按 <kbd>s</kbd> 键打开排序菜单，可按以下维度排序：
- 优先级
- 创建时间
- 到期日期
- 标题字母序
