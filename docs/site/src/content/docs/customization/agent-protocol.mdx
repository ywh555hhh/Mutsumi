...
Mutsumi ships 5 bundled skills that follow the [Agent Skills open standard](https://agentskills.io). When installed, agents discover them automatically — no manual prompt pasting needed.
...
Step 1: `ensure_ssot()` copies bundled skills → `~/.mutsumi/skills/`.
...
<Aside type="tip">
Skills are the **recommended** integration method. The manual prompt template (below) is provided as a fallback for agents that don't support the Agent Skills standard.
</Aside>
