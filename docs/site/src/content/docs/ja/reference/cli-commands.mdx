...
| モード | 動作 |
|--------|------|
| `skills` | 設定に優先 Agent を記録するのみ。ファイルは変更されない。 |
| `skills+project-doc` | 設定を保存し、Agent のプロジェクトドキュメント（`CLAUDE.md`、`AGENTS.md` など）に統合手順を追記。 |
| `snippet` | 統合プロンプトを標準出力に表示。手動ペースト用。 |
