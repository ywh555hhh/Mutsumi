...
| **tmux** | ペイン分割、セッション永続化 | `apt install tmux` / `brew install tmux` |
...
<Tabs>
  <TabItem label="Windows">
    ...
    Windows Terminal のデフォルトプロファイルを **Ubuntu (WSL)** に設定してください。

    ### ステップ 3：WSL 内にツールをインストール
...
<Tabs>
  ...
  <TabItem label="PowerShell (WSL)">
    ```powershell
    # mu.ps1 — WSL を呼び出す Windows ショートカット
...
<Steps>
1. **ホットキーを押す** — `Win+`` / `Ctrl+`` / `F12` — ターミナルがドロップダウン
...
| WSL パスの問題 | `wslpath` でパス変換。PowerShell スクリプトは自動処理済み |
