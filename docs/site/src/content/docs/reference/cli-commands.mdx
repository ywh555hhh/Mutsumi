...
|--------|-------|-------------|---------|
| `--path PATH` | `-p` | Path to task file | `./mutsumi.json` (fallback: `./tasks.json`) |
| `--watch PATH` | `-w` | Additional task file paths to watch (repeatable) | — |
...
| `--scope` | `-s` | choice | `day`, `week`, `month`, `inbox` | Config `default_scope` |
...
| Option | Short | Type | Values | Default |
|--------|-------|------|--------|---------|
| `--scope` | `-s` | choice | `day`, `week`, `month`, `inbox` | all |
| `--done / --no-done` | | flag | | show all |
...
| Option | Short | Type | Values | Description |
|--------|-------|------|--------|-------------|
| `--title` | | string | | New task title |
| `--priority` | `-P` | choice | `high`, `normal`, `low` | New priority |
| `--scope` | `-s` | choice | `day`, `week`, `month`, `inbox` | New scope |
| `--tags` | `-t` | string | Comma-separated | Replace tags |
| `--description` | `-d` | string | Free text | New description |
...
| `--personal` | Initialize personal task file at `~/.mutsumi/mutsumi.json` |
...
| Option | Description |
|--------|-------------|
| (none) | Rename `tasks.json` → `mutsumi.json` in current directory |
...
Register a project directory as a source. The directory should contain (or will contain) a `mutsumi.json`.
...
Validates `mutsumi.json` (or the file specified by `--path`) against the expected schema. Reports missing required fields, invalid enum values, and malformed JSON.
