...
Set up Mutsumi integration for an AI agent. Installs [Agent Skills](https://agentskills.io) into the agent's skill directory via symlinks.
...
| Mode | Behavior |
|------|----------|
| `skills` | Install 5 Mutsumi skill files (`mutsumi-manage`, `mutsumi-track`, `mutsumi-plan`, `mutsumi-report`, `mutsumi-context`) as symlinks from `~/.mutsumi/skills/` into the agent's skill directory. |
| `skills+project-doc` | Install skills AND append a `## Mutsumi Task Integration` section to the agent's project doc (`CLAUDE.md`, `AGENTS.md`, etc.). |
| `snippet` | Print the integration prompt to stdout for manual pasting. No files installed. |

### Installed Skills

| Skill | Purpose | Auto-triggered? |
|-------|---------|-----------------|
| `mutsumi-manage` | Task CRUD (add, edit, done, remove) | No — user asks |
| `mutsumi-track` | Auto-update task progress while working | Yes |
| `mutsumi-plan` | Break goals into subtasks | No — user asks |
| `mutsumi-report` | Generate status summary | No — user asks |
| `mutsumi-context` | Load task board context at session start | Yes |
...
# Install skills for Claude Code (default mode)
mutsumi setup --agent claude-code

# Install skills + inject project doc into CLAUDE.md
mutsumi setup --agent claude-code --mode skills+project-doc

# Print prompt for manual use (Aider, custom agents)
...
Running setup again is safe -- it replaces stale symlinks and never duplicates.
